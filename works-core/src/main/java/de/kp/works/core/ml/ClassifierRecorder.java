...
public class ClassifierRecorder extends AbstractRecorder {
	...
	protected String getBestModelFsPath(Table table, String algorithmName, String modelName, String modelStage) {
		...
		return scanner.bestClassifier(table, algorithmName, modelName, modelStage);
	...
	protected void setMetadata(long ts, Table table, String algorithmName, String modelName, String modelPack,
		...
		table.put(row.add("accuracy", accuracy).add("f1", f1).add("hammingLoss", hammingLoss)
				.add("weightedFMeasure", weightedFMeasure).add("weightedPrecision", weightedPrecision)
				.add("weightedRecall", weightedRecall).add("weightedFalsePositiveRate", weightedFalsePositiveRate)
				.add("weightedTruePositiveRate", weightedTruePositiveRate));
