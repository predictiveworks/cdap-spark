...
class JsonOptions(
  ...
  def this(
    ...
    defaultColumnNameOfCorruptRecord: String) = {
      this(
        CaseInsensitiveMap(parameters.toMap),
        defaultTimeZoneId,
   ...
   */
  val caseSensitive =     
    parameters.get("caseSensitive").map(_.toBoolean).getOrElse(false)
  ...
  val ignoreNullFields = parameters.get("ignoreNullFields").map(_.toBoolean).getOrElse(false)
