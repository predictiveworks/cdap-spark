...
import org.apache.spark.sql.catalyst.analysis.TypeCoercionExt
 ...
 * __KUP__
 * 
 * This class is extracted from v2.4.x and adjusted to the
 * dependencies of earlier versions, especially v2.1.3
 */
class JsonInferSchema(options: JsonOptions) extends Serializable {

  private val decimalParser = ExprUtils.getDecimalParser(options.locale)
  private val jsonFactory = new JsonFactory()
  
  /* 
   * Evaluate the provided infer options to determine
   * whether a case sensitive or insensitive analysis
   * must be performed
   */
  ...
  def inferFromJava(javaSamples:java.util.List[String]): StructType = {
    val scalaSamples = javaSamples.asScala.toList
    infer(scalaSamples)
  }
  /**
   ...
   */
  def infer[String](
      json: List[String]): StructType = {
    
    val parseMode = options.parseMode
    val columnNameOfCorruptRecord = options.columnNameOfCorruptRecord

    val typeMerger = JsonInferSchema.compatibleRootType(resolver, columnNameOfCorruptRecord, parseMode)
    
    val inferredTypes = json.map { row =>
        try {
            /*
             * Testing issues 2019-12-9: It is important to define the JsonParser
             * here to circument annoying ClassLoader and Class Cast issues
             */
            val parser = jsonFactory.createParser(row.toString)
          ...
          case  e @ (_: RuntimeException | _: JsonProcessingException) => {
            ...
            case DropMalformedMode =>
              None
            case FailFastMode =>
              throw new IllegalArgumentException("Malformed records are detected in schema inference. " +
    ...
    inferredTypes.foreach(inferredType => {
      if (inferredType.isDefined)
          rootType = typeMerger(rootType, inferredType.get)
    })
    ...
    token match {
      ...
      case START_ARRAY =>
        ...
        while (nextUntil(parser, END_ARRAY)) {
          elementType = JsonInferSchema.compatibleType(resolver, elementType, inferField(parser))
        }
...
object JsonInferSchema {
  ...
  def compatibleRootType(
    ...
    case (ArrayType(ty1, _), ty2) =>
      compatibleRootType(resolver, columnNameOfCorruptRecords, parseMode)(ty1, ty2)
    ...
    case (ty1, ty2) => compatibleType(resolver, ty1, ty2)
  }
   ...
   */
  def compatibleType(resolver: Resolver, t1: DataType, t2: DataType): DataType = {
    new TypeCoercionExt(resolver).findTightestCommonType(t1, t2).getOrElse {
      ...
      (t1, t2) match {
        ...
        case (StructType(fields1), StructType(fields2)) =>
          ...
          assert(isSorted(fields1),
            s"Struct fields were not sorted: ${fields1.toSeq}")
          assert(isSorted(fields2),
            s"Struct fields were not sorted: ${fields2.toSeq}")
          ...
          while (f1Idx < fields1.length && f2Idx < fields2.length) {
            ...
            if (comp == 0) {
              val dataType = compatibleType(resolver, fields1(f1Idx).dataType, fields2(f2Idx).dataType)
        ...
        case (ArrayType(elementType1, containsNull1), ArrayType(elementType2, containsNull2)) =>
          ArrayType(compatibleType(resolver, elementType1, elementType2), containsNull1 || containsNull2)
        ...
        case (t1: IntegralType, t2: DecimalType) =>
          compatibleType(resolver, DataTypeUtil.DecimalforType(t1), t2)
        case (t1: DecimalType, t2: IntegralType) =>
          compatibleType(resolver, t1, DataTypeUtil.DecimalforType(t2))
