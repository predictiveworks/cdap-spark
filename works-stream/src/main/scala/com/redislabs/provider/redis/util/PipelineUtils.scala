...
object PipelineUtils {
   ...
   */
  def mapWithPipeline[A](conn: Jedis, items: TraversableOnce[A])(f: (Pipeline, A) => Unit)(implicit readWriteConfig: ReadWriteConfig): Seq[AnyRef] = {
   ...
   * @param f               function to applied for each item in the sequence
   */
  def foreachWithPipeline[A](conn: Jedis, items: TraversableOnce[A])(f: (Pipeline, A) => Unit)(implicit readWriteConfig: ReadWriteConfig): Unit = {
    // iterate over items and create new pipelines periodically
    var i = 0
    var pipeline = conn.pipelined()
    for (x <- items) {
  ...
  /**
   * Executes a pipeline function for each item in the sequence. Doesn't sync 
   * and return the last pipeline after all operations are executed. 
   * 
   * Allows to execute more operations with the returned pipeline. The client 
   * is responsible of syncing the returned pipeline.
   *
   ...
   */
  def foreachWithPipelineNoLastSync[A](conn: Jedis, items: TraversableOnce[A])(f: (Pipeline, A) => Unit)(implicit readWriteConfig: ReadWriteConfig): Pipeline = {
    // iterate over items and create new pipelines periodically
    var i = 0
    var pipeline = conn.pipelined()
    for (x <- items) {
