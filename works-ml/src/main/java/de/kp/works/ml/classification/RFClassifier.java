	...
	public static class RFClassifierConfig extends BaseClassifierConfig {
		...
		public void validate() {
			...
			if (!Strings.isNullOrEmpty(modelName)) {
				throw new IllegalArgumentException("[RFClassifierConfig] The model name must not be empty.");
			}
			if (!Strings.isNullOrEmpty(featuresCol)) {
				throw new IllegalArgumentException("[RFClassifierConfig] The name of the field that contains the feature vector must not be empty.");
			}
			if (!Strings.isNullOrEmpty(labelCol)) {
				throw new IllegalArgumentException("[RFClassifierConfig] The name of the field that contains the label value must not be empty.");
			}
			
			/** PARAMETERS **/
			if (maxBins < 2)
				throw new IllegalArgumentException("[RFClassifierConfig] The maximum bins must be at least 2.");

			if (maxDepth < 0)
				throw new IllegalArgumentException("[RFClassifierConfig] The maximum depth must be nonnegative.");
			
			if (minInfoGain < 0D)
				throw new IllegalArgumentException("[RFClassifierConfig] The minimum information gain must be at least 0.0.");
			
			if (minInfoGain < 1)
				throw new IllegalArgumentException("[RFClassifierConfig] The number of trees must be at least 1.");
