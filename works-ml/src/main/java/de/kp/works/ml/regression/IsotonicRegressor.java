...
public class IsotonicRegressor extends BaseRegressorSink {
	...
	public void configurePipeline(PipelineConfigurer pipelineConfigurer) throws IllegalArgumentException {
		...
		validateSchema(inputSchema, config, IsotonicRegressor.class.getName());
   ...
   */
		public void validate() {
			...
			if (!Strings.isNullOrEmpty(modelName)) {
				throw new IllegalArgumentException("[IsotonicConfig] The model name must not be empty.");
			}
			if (!Strings.isNullOrEmpty(featuresCol)) {
				throw new IllegalArgumentException("[IsotonicConfig] The name of the field that contains the feature vector must not be empty.");
			}
			if (!Strings.isNullOrEmpty(labelCol)) {
				throw new IllegalArgumentException("[IsotonicConfig] The name of the field that contains the label value must not be empty.");
			}
