...
public class NBPredictor extends BasePredictorCompute {
	...
	public void initialize(SparkExecutionPluginContext context) throws Exception {
		((NBPredictorConfig)config).validate();

		modelFs = SparkMLManager.getClassificationFS(context);
		modelMeta = SparkMLManager.getClassificationMeta(context);

		classifier = new NBClassifierManager().read(modelFs, modelMeta, config.modelName);
		if (classifier == null)
	...
	/**
	 * This method computes predictions either by applying a trained Naive Bayes
	 * classification model; as a result, the source dataset is enriched by an 
	 * extra column (predictionCol) that specifies the target variable in form
	 * of a Double value
	 */
	@Override
